"""Architecture blocks, modified from the original CAMNet GitHub repo:

https://github.com/niopeng/CAM-Net/blob/5d5d68fe921161e68bdc2c603423e7c10c98e0ce/code/models/modules/block.py
...
class NoiseEncoderBlock(nn.Module):
    """
    Noise Encoder Blocks
    style: n * conv2 layers
    LayerNorm: https://pytorch.org/docs/stable/generated/torch.nn.LayerNorm.html
    Normal_truncated_initializer: https://pytorch.org/docs/stable/_modules/torch/nn/init.html#constant_
    """

    def __init__(self, in_ch, out_ch, input_size, kernel_sizes=[3, 3, 1]):
        super(NoiseEncoderBlock, self).__init__()
        self.nb = len(kernel_sizes)
        input_size.insert(0, out_ch)
        for i, k_size in enumerate(kernel_sizes):
            if i == 0:  # first layer
                self.add_module("noise_conv_block_%d" % i,
                                conv_block_noise(in_nc=in_ch, out_nc=out_ch, kernel_size=k_size, act_type="leakyrelu"))
                self.add_module("noise_layer_norm_%d" % i, nn.LayerNorm(input_size))
            elif i == len(kernel_sizes) - 1:  # last layer
                self.add_module("noise_conv_block_%d" % i,
                                conv_block_noise(in_nc=out_ch, out_nc=in_ch, kernel_size=k_size, act_type=None))
            else:  # middle layer
                self.add_module("noise_conv_block_%d" % i,
                                conv_block_noise(in_nc=out_ch, out_nc=out_ch, kernel_size=k_size, act_type="leakyrelu"))
                self.add_module("noise_layer_norm_%d" % i, nn.LayerNorm(input_size))

    def forward(self, x):
        for i in range(self.nb):
            x = getattr(self, "noise_conv_block_%d" % i)(x)
            if i != self.nb - 1:
                x = getattr(self, "noise_layer_norm_%d" % i)(x)
        return x
