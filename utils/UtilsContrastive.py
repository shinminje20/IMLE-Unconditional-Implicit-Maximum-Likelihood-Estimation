...
def lars_params(model):
    """Returns the param_groups for [model] for use in a LARS-wrapped optimizer.
    """
    return [
        {"params": [p for n,p in model.named_parameters() if "bn" in n],
        "weight_decay": 1e-6, "layer_adaption": False
        },
        {"params": [p for n,p in model.named_parameters() if not "bn" in n],
        "weight_decay": 1e-6, "layer_adaption": True}]
