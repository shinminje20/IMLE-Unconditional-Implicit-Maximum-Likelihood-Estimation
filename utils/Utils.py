...
def generator_folder(args, ignore_conflict=True):
    ...
    folder = f"{project_dir}/generators/{args.data}-bs{args.bs}-decay_{args.lr_decay}-epochs{args.epochs}-grayscale{args.grayscale}-ipc{args.ipc}-lr{args.lr}-mask_frac{args.mask_frac}-mask_res{args.mask_res}-ns{'_'.join([str(n) for n in args.ns])}-res{'_'.join([str(r) for r in args.res])}-seed{args.seed}" + suffix_str(args)   
