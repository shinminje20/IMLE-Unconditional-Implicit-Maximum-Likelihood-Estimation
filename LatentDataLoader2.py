...
from torch.utils.data import IterDataPipe, DataLoader, Subset
...
class Latent_DataLoader(Generic[T_co]):
    r"""
    ...
    def __init__(self, dataset: Dataset[T_co], model: Optional[object] = None, corruptor: Optional[object] = None, z_gen: Optional[object] = None, loss_fn: Optional[object] = None, num_samples: Optional[int] = 16, sample_parallelism: Optional[int] = 16, 
                code_bs: Optional[int] = 512, subsample_size=None, num_iteration=1, batch_size: Optional[int] = 1,
    ...
    def _get_iterator(self) -> '_BaseDataLoaderIter':
        if self.num_workers == 0:
            ...
            for i in range(self.num_iteration):
                ...
                # yield _MultiProcessingDataLoaderIter(loader)
    ...
    def __setattr__(self, attr, val):
        ...
        super(Latent_DataLoader, self).__setattr__(attr, val)
    ...
    def __iter__(self) -> '_BaseDataLoaderIter':
        ...
        if self.persistent_workers and self.num_workers > 0:
            ...
            print("=-=-=-=-=-=-=-=-=--=-=-=-=-=-= return self._get_iterator()")
            print("if self._iterator is None:: ", self._iterator is None)
            if self._iter_counter != 0 and self._iter_counter % (self.num_iteration * self.subsample_size // self.batch_size) == 0:
                self._iterator = self._get_iterator()
    ...
    def __len__(self) -> int:
        if self._dataset_kind == _DatasetKind.Iterable:
            ...
            return self.num_iteration * self.subsample_size // self.batch_size
...
class _BaseDataLoaderIter(object):
    def __init__(self, loader: Latent_DataLoader) -> None:
        ...
        self._num_yielded = 0
        self._profile_name = "enumerate(Latent_DataLoader)#{}.__next__".format(self.__class__.__name__)

        # print(" ========================= Inside BatchLoader init ========================= ")
        # print("length of dataloader : ", len(loader))
        # for i, (cx, codes, ys) in enumerate(loader):
        #     print("i, (cx, codes, ys): ", i, len(cx), len(codes), len(ys))
        # print(" ========================= ========================= ========================= ")
    def __iter__(self) -> '_BaseDataLoaderIter':
        return self
