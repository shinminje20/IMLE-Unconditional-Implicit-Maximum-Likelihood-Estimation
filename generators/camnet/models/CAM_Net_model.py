...
class CAM_NetModel(BaseModel):
    def __init__(self, opt):
        super(CAM_NetModel, self).__init__(opt)
        train_opt = opt['train']
        self.task = opt['task']
    ...
    def _get_feature_loss(self, gen_img_rgb, real_img_rgb):
        ...
        print("netF DEVICE", self.netF.device)
    ...
    def get_loss(self, level=-1):
        ...
        with torch.no_grad():
            ...
            if self.rarity_masks is not None:
                img_out_rgb *= self.rarity_masks[level]
                real_img *= self.rarity_masks[level]
    ...
    def optimize_parameters(self, step, inter_supervision=False):
        ...
        if inter_supervision:
            for i, output in enumerate(outputs):
                ...
                if self.rarity_masks is not None:
                    img_out_rgb *= self.rarity_masks[i]  # generated image
                    real_img *= self.rarity_masks[i]

                if self.cri_pix:  # pixel loss
                    l_g_total += self.l_pix_w * self.cri_pix(img_out_rgb, real_img)
                else:
            ...
            if self.rarity_masks is not None:
                img_out_rgb *= self.rarity_masks[-1]
                real_img *= self.rarity_masks[-1]
    ...
    def get_current_visuals(self, need_HR=True):
        out_dict = OrderedDict()
        out_dict['LR'] = self.network_input[-1].detach()[0].float().cpu()  # we don't need this in colorization
        out_dict['HR_pred'] = self.pred[-1].detach()[0].float().cpu()  # line 189, predicted image in rgb
        if need_HR:
