...
class ColorizationSuperResolutionDataset(data.Dataset):
    """ For now, identical to super-resolution, except network inputs have a
    mean taken over the channels axis.
    """
    ...
    def __getitem__(self, index):
        ...
        if self.opt['phase'] == 'train':
            ...
            img_LR, img_HR, img_D1, img_D2, img_D3 = util.augment(
                [img_LR, img_HR, img_D1, img_D2, img_D3],
                self.opt['use_flip'],
                self.opt['use_rot'])
        ...
        print("LR SHAPE", img_LR.shape)
        assert False


        if LR_path is None:
            LR_path = HR_path
        res = {'network_input': [img_LR],
               'HR': img_HR,
               'LR_path': LR_path,
               'HR_path': HR_path,
               'D1': img_D1,
